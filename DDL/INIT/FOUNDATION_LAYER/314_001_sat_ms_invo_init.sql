CREATE OR REPLACE FUNCTION "moto_scn01_proc"."sat_ms_invo_init"() 
RETURNS void 
LANGUAGE 'plpgsql' 
...
BEGIN -- sat_tgt
	...
	TRUNCATE TABLE "moto_scn01_fl"."sat_ms_invoices"  CASCADE;
	...
	( 
		SELECT 
			...
			, UPPER(ENCODE(DIGEST(COALESCE(RTRIM( UPPER(REPLACE(COALESCE(TRIM( TO_CHAR("stg_inr_src"."invoice_date", 
				'DD/MM/YYYY'::varchar)),'~'),'#','\' || '#'))|| '#' ||  UPPER(REPLACE(COALESCE(TRIM( "stg_inr_src"."amount"::text),'~'),'#','\' || '#'))|| '#' ||  UPPER(REPLACE(COALESCE(TRIM( "stg_inr_src"."discount"::text),'~'),'#','\' || '#'))|| '#','#' || '~'),'~') ,'MD5'),'HEX')) AS "hash_diff"
			, 'N'::text AS "delete_flag"
			, "stg_inr_src"."trans_timestamp" AS "trans_timestamp"
			, "stg_inr_src"."invoice_number" AS "invoice_number"
			, "stg_inr_src"."invoice_date" AS "invoice_date"
	...
	SELECT 
		...
		, "stg_src"."invoice_date" AS "invoice_date"
