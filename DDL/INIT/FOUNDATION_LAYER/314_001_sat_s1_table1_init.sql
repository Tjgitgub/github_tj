CREATE OR REPLACE FUNCTION "project1_proc"."sat_s1_table1_init"() 
RETURNS void 
LANGUAGE 'plpgsql' 
...
BEGIN -- sat_tgt
	...
	TRUNCATE TABLE "dv_0114_fl"."sat_s1_table1"  CASCADE;
	...
	( 
		SELECT 
			  "stg_inr_src"."table1_hkey" AS "table1_hkey"
			, "stg_inr_src"."load_date" AS "load_date"
			, TO_TIMESTAMP('31/12/2999 23:59:59' , 'DD/MM/YYYY HH24:MI:SS'::varchar) AS "load_end_date"
			, "stg_inr_src"."load_cycle_id" AS "load_cycle_id"
			, UPPER(ENCODE(DIGEST(COALESCE(RTRIM( REPLACE(COALESCE(TRIM( "stg_inr_src"."table1_id"::text),'~'),'#','\' || 
				'#')|| '#' ||  REPLACE(COALESCE(TRIM( "stg_inr_src"."some_attr"::text),'~'),'#','\' || '#')|| '#','#' || '~'),'~') ,'MD5'),'HEX')) AS "hash_diff"
			, 'N'::text AS "delete_flag"
			, "stg_inr_src"."other_attr" AS "other_attr"
			, "stg_inr_src"."table1_id" AS "table1_id"
			, "stg_inr_src"."some_attr" AS "some_attr"
			, ROW_NUMBER()OVER(PARTITION BY "stg_inr_src"."table1_hkey" ORDER BY "stg_inr_src"."load_date") AS "dummy"
		FROM "source1_stg"."table1" "stg_inr_src"
	)
	SELECT 
		  "stg_src"."table1_hkey" AS "table1_hkey"
		...
		, "stg_src"."table1_id" AS "table1_id"
