CREATE OR REPLACE FUNCTION "moto_scn01_proc"."ext_ms_addresses_init"() 
RETURNS void 
LANGUAGE 'plpgsql' 
...
BEGIN -- ext_tgt
	...
	( 
		SELECT 
			...
			, "ini_src"."coordinates" AS "coordinates"
			, "ini_src"."update_timestamp" AS "update_timestamp"
		FROM "moto_sales_scn01"."addresses" "ini_src"
	...
	( 
		SELECT 
			...
			, "load_init_data"."coordinates" AS "coordinates"
		...
		SELECT 
			...
			, CAST("mex_ext_src"."attribute_gps" AS gps) AS "coordinates"
	...
	( 
		SELECT 
			...
			, CASE WHEN TRIM("prep_excep"."street_name")= '' OR "prep_excep"."street_name" IS NULL THEN "mex_src"."key_attribute_varchar" ELSE UPPER(REPLACE(TRIM("prep_excep"."street_name")
				,'#','\' || '#'))END AS "street_name_bk"
			, COALESCE(UPPER( "prep_excep"."street_number"::text),"mex_src"."key_attribute_numeric") AS "street_number_bk"
			, CASE WHEN TRIM("prep_excep"."postal_code")= '' OR "prep_excep"."postal_code" IS NULL THEN "mex_src"."key_attribute_varchar" ELSE UPPER(REPLACE(TRIM("prep_excep"."postal_code")
				,'#','\' || '#'))END AS "postal_code_bk"
			, CASE WHEN TRIM("prep_excep"."city")= '' OR "prep_excep"."city" IS NULL THEN "mex_src"."key_attribute_varchar" ELSE UPPER(REPLACE(TRIM("prep_excep"."city")
				,'#','\' || '#'))END AS "city_bk"
			...
			, "prep_excep"."coordinates" AS "coordinates"
