CREATE OR REPLACE FUNCTION "moto_scn01_proc"."stg_mm_party_init"() 
RETURNS void 
LANGUAGE 'plpgsql' 
...
BEGIN -- stg_tgt
	...
	SELECT 
		...
		, "ext_src"."name_bk" || '#' ||  "ext_src"."birthdate_bk" || '#' ||  "ext_src"."gender_bk" || '#' ||  "ext_src"."party_type_code_bk" AS "customers_bk"
		...
		, CASE WHEN "find_bk_fk1"."parent_party_number" IS NULL THEN 1 ELSE 0 END AS "error_code_cust_cust_parentpartynumber"
		, CASE WHEN "find_bk_fk2"."address_number" IS NULL THEN 1 ELSE 0 END AS "error_code_cust_addr"
...
BEGIN -- ext_err_tgt
	...
	WHERE ( "stg_err_src"."error_code_cust_cust_parentpartynumber" > 0 OR  "stg_err_src"."error_code_cust_addr" > 0)AND "stg_err_src"."load_cycle_id" >= 0 AND "err_mex_src"."record_type" = 'N'
	;
END;
