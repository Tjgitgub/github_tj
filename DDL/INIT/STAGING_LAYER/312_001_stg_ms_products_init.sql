CREATE OR REPLACE FUNCTION "moto_scn01_proc"."stg_ms_products_init"() 
RETURNS void 
LANGUAGE 'plpgsql' 
...
BEGIN -- stg_tgt
	...
	SELECT 
		...
		, CASE WHEN "find_bk_fk1"."replacement_product_id" IS NULL THEN 1 ELSE 0 END AS "error_code_prod_prod_rpid"
...
BEGIN -- ext_err_tgt
	...
	WHERE ( "stg_err_src"."error_code_prod_prod_rpid" > 0)AND "stg_err_src"."load_cycle_id" >= 0
	;
END;
