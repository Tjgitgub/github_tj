CREATE OR REPLACE FUNCTION "moto_scn01_proc"."stg_ms_invoicelines_init"() 
RETURNS void 
LANGUAGE 'plpgsql' 
...
BEGIN -- stg_tgt
	...
	SELECT 
		  UPPER(ENCODE(DIGEST( "ext_src"."invoice_number_bk" || '#' ||  "ext_src"."invoice_line_number_bk" || '#' ,'MD5')
			,'HEX')) AS "invoice_lines_hkey"
		...
		, CASE WHEN "find_bk_fk1"."product_id" IS NULL THEN 1 ELSE 0 END AS "error_code_inli_prod"
		, CASE WHEN "find_bk_fk3"."part_id" IS NULL THEN 1 ELSE 0 END AS "error_code_inli_part"
...
BEGIN -- ext_err_tgt
	...
	WHERE ( "stg_err_src"."error_code_inli_prod" > 0 OR  "stg_err_src"."error_code_inli_part" > 0)AND "stg_err_src"."load_cycle_id" >= 0
	;
END;
