CREATE OR REPLACE FUNCTION "moto_scn01_proc"."stg_ms_customers_init"() 
RETURNS void 
LANGUAGE 'plpgsql' 
...
BEGIN -- stg_tgt
	...
	SELECT 
		...
		, CASE WHEN "find_bk_fk1"."customer_ship_to_address_id" IS NULL THEN 1 ELSE 0 END AS "error_code_cust_addr_iats"
		, CASE WHEN "find_bk_fk2"."customer_invoice_address_id" IS NULL THEN 1 ELSE 0 END AS "error_code_cust_addr_ciai"
...
BEGIN -- ext_err_tgt
	...
	WHERE ( "stg_err_src"."error_code_cust_addr_iats" > 0 OR  "stg_err_src"."error_code_cust_addr_ciai" > 0)AND "stg_err_src"."load_cycle_id" >= 0 AND "err_mex_src"."record_type" = 'N'
	;
END;
