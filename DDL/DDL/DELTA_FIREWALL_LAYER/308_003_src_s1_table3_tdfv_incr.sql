...
DROP VIEW IF EXISTS "source1_dfv"."vw_table3";
CREATE  VIEW "source1_dfv"."vw_table3"  AS 
	WITH "delta_view_filter" AS 
	( 
		SELECT 
			  'S' ::text AS "record_type"
			, "cdc_src"."table3_id" AS "table3_id"
			, "cdc_src"."some_attr" AS "some_attr"
			, "cdc_src"."other_attr" AS "other_attr"
		FROM "source1_cdc"."cdc_table3" "cdc_src"
	)
	, "delta_view" AS 
	( 
		SELECT 
			  "delta_view_filter"."record_type" AS "record_type"
			, "delta_view_filter"."table3_id" AS "table3_id"
	...
	( 
		SELECT 
			...
			, COALESCE("delta_view"."table3_id", CAST("mex_bk_src"."key_attribute_integer" AS INTEGER)) AS "table3_id"
	...
	SELECT 
		...
		, "prepjoinbk"."table3_id" AS "table3_id"
