CREATE OR REPLACE FUNCTION "project1_proc"."sat_s1_table2_incr"() 
RETURNS void 
LANGUAGE 'plpgsql' 
...
BEGIN -- sat_temp_tgt
	...
	TRUNCATE TABLE "dv_0114_fl"."sat_s1_table2_tmp"  CASCADE;

	INSERT INTO "dv_0114_fl"."sat_s1_table2_tmp"(
		 "table2_hkey"
		,"load_date"
		,"load_end_date"
		,"load_cycle_id"
	...
	( 
		SELECT DISTINCT 
 			  "stg_dis_src"."table2_hkey" AS "table2_hkey"
			, "stg_dis_src"."load_cycle_id" AS "load_cycle_id"
		FROM "source1_stg"."table2" "stg_dis_src"
		WHERE  "stg_dis_src"."record_type" = 'S'
	)
	, "temp_table_set" AS 
	( 
		SELECT 
			  "stg_temp_src"."table2_hkey" AS "table2_hkey"
			...
			, "stg_temp_src"."table2_id" AS "table2_id"
			, "stg_temp_src"."some_attr" AS "some_attr"
			, "stg_temp_src"."other_attr" AS "other_attr"
		FROM "source1_stg"."table2" "stg_temp_src"
		WHERE  "stg_temp_src"."record_type" = 'S'
		UNION ALL 
		SELECT 
			  "sat_src"."table2_hkey" AS "table2_hkey"
			...
			, "sat_src"."table2_id" AS "table2_id"
			, "sat_src"."some_attr" AS "some_attr"
			, "sat_src"."other_attr" AS "other_attr"
		FROM "dv_0114_fl"."sat_s1_table2" "sat_src"
		INNER JOIN "dist_stg" "dist_stg" ON  "sat_src"."table2_hkey" = "dist_stg"."table2_hkey"
		WHERE  "sat_src"."load_end_date" = TO_TIMESTAMP('31/12/2999 23:59:59' , 'DD/MM/YYYY HH24:MI:SS'::varchar)
	)
	SELECT 
		  "temp_table_set"."table2_hkey" AS "table2_hkey"
		...
		, CASE WHEN "temp_table_set"."source" = 'STG' AND "temp_table_set"."delete_flag"::text || "temp_table_set"."hash_diff"::text =
			LAG( "temp_table_set"."delete_flag"::text || "temp_table_set"."hash_diff"::text,1)OVER(PARTITION BY "temp_table_set"."table2_hkey" ORDER BY "temp_table_set"."load_date","temp_table_set"."origin_id")THEN 1 ELSE 0 END AS "equal"
		, "temp_table_set"."hash_diff" AS "hash_diff"
		, "temp_table_set"."delete_flag" AS "delete_flag"
		, "temp_table_set"."table2_id" AS "table2_id"
...
BEGIN -- sat_inur_tgt
	...
	SELECT 
		  "sat_temp_src_inur"."table2_hkey" AS "table2_hkey"
		...
		, "sat_temp_src_inur"."table2_id" AS "table2_id"
		, "sat_temp_src_inur"."some_attr" AS "some_attr"
		, "sat_temp_src_inur"."other_attr" AS "other_attr"
	FROM "dv_0114_fl"."sat_s1_table2_tmp" "sat_temp_src_inur"
...
BEGIN -- sat_ed_tgt
	...
	( 
		SELECT 
			  "sat_temp_src_us"."table2_hkey" AS "table2_hkey"
			, "sat_temp_src_us"."load_date" AS "load_date"
			, COALESCE(LEAD("sat_temp_src_us"."load_date",1)OVER(PARTITION BY "sat_temp_src_us"."table2_hkey" ORDER BY "sat_temp_src_us"."load_date")
				, TO_TIMESTAMP('31/12/2999 23:59:59', 'DD/MM/YYYY HH24:MI:SS'::varchar)) AS "load_end_date"
		FROM "dv_0114_fl"."sat_s1_table2_tmp" "sat_temp_src_us"
		WHERE  "sat_temp_src_us"."equal" = 0
	)
	, "filter_load_end_date" AS 
	( 
		SELECT 
			  "calc_load_end_date"."table2_hkey" AS "table2_hkey"
	...
	UPDATE "dv_0114_fl"."sat_s1_table2" "sat_ed_tgt"
	SET 
		 "load_end_date" =  "filter_load_end_date"."load_end_date"
	FROM  "filter_load_end_date"
	WHERE "sat_ed_tgt"."table2_hkey" =  "filter_load_end_date"."table2_hkey"
