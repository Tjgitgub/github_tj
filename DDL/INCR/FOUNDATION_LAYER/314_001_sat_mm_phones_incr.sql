CREATE OR REPLACE FUNCTION "moto_scn01_proc"."sat_mm_phones_incr"() 
RETURNS void 
LANGUAGE 'plpgsql' 
...
BEGIN -- sat_inr_tgt
	...
	INSERT INTO "moto_scn01_fl"."sat_mm_phones"(
		 "contacts_hkey"
		,"load_date"
		,"load_end_date"
		,"load_cycle_id"
	...
	SELECT 
		...
		, UPPER(ENCODE(DIGEST(COALESCE(RTRIM( UPPER(REPLACE(COALESCE(TRIM( "stg_inr_src"."phone_number"),'~'),'#','\' || 
		...
		, "stg_inr_src"."phone_number" AS "phone_number"
		, "stg_inr_src"."update_timestamp" AS "update_timestamp"
	FROM "moto_mktg_scn01_stg"."phones" "stg_inr_src"
	LEFT OUTER JOIN "moto_scn01_fl"."sat_mm_phones" "sat_inr_src" ON  "stg_inr_src"."contacts_hkey" = "sat_inr_src"."contacts_hkey" AND "stg_inr_src"."load_date" = "sat_inr_src"."load_date"
...
BEGIN -- sat_temp_tgt
	...
	TRUNCATE TABLE "moto_mktg_scn01_stg"."sat_mm_phones_tmp"  CASCADE;

	INSERT INTO "moto_mktg_scn01_stg"."sat_mm_phones_tmp"(
		 "contacts_hkey"
		,"load_date"
		,"load_end_date"
		,"load_cycle_id"
	...
	( 
		...
		FROM "moto_mktg_scn01_stg"."phones" "stg_dis_src"
	...
	( 
		SELECT 
			...
			, UPPER(ENCODE(DIGEST(COALESCE(RTRIM( UPPER(REPLACE(COALESCE(TRIM( "stg_temp_src"."phone_number"),'~'),'#','\' || 
				'#'))|| '#','#' || '~'),'~') ,'MD5'),'HEX')) AS "hash_diff"
			...
			, "stg_temp_src"."phone_number" AS "phone_number"
			, "stg_temp_src"."update_timestamp" AS "update_timestamp"
		FROM "moto_mktg_scn01_stg"."phones" "stg_temp_src"
		...
		SELECT 
			...
			, "sat_src"."phone_number" AS "phone_number"
			, "sat_src"."update_timestamp" AS "update_timestamp"
		FROM "moto_scn01_fl"."sat_mm_phones" "sat_src"
	...
	SELECT 
		...
		, "temp_table_set"."phone_number" AS "phone_number"
...
BEGIN -- sat_inur_tgt
	...
	SELECT 
		...
		, "sat_temp_src_inur"."phone_number" AS "phone_number"
		, "sat_temp_src_inur"."update_timestamp" AS "update_timestamp"
	FROM "moto_mktg_scn01_stg"."sat_mm_phones_tmp" "sat_temp_src_inur"
...
BEGIN -- sat_ed_tgt
	...
	( 
		SELECT 
			...
			, COALESCE(LEAD("sat_temp_src_us"."load_date",1)OVER(PARTITION BY "sat_temp_src_us"."contacts_hkey" ORDER BY "sat_temp_src_us"."load_date")
				, TO_TIMESTAMP('31/12/2399 23:59:59.000000', 'DD/MM/YYYY HH24:MI:SS.US'::varchar)) AS "load_end_date"
		FROM "moto_mktg_scn01_stg"."sat_mm_phones_tmp" "sat_temp_src_us"
	...
	UPDATE "moto_scn01_fl"."sat_mm_phones" "sat_ed_tgt"
	SET 
