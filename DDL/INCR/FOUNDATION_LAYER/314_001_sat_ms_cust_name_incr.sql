CREATE OR REPLACE FUNCTION "moto_scn01_proc"."sat_ms_cust_name_incr"() 
RETURNS void 
LANGUAGE 'plpgsql' 
...
BEGIN -- sat_temp_tgt
	...
	TRUNCATE TABLE "moto_sales_scn01_stg"."sat_ms_customers_name_tmp"  CASCADE;

	INSERT INTO "moto_sales_scn01_stg"."sat_ms_customers_name_tmp"(
		 "customers_hkey"
		,"load_date"
		,"load_end_date"
		,"load_cycle_id"
	...
	( 
		SELECT 
			...
			, 'STG' AS "source"
			, 1 AS "origin_id"
			, UPPER(ENCODE(DIGEST(COALESCE(RTRIM( UPPER(REPLACE(COALESCE(TRIM( "stg_temp_src"."first_name"),'~'),'#','\' || 
				'#'))|| '#' ||  UPPER(REPLACE(COALESCE(TRIM( "stg_temp_src"."last_name"),'~'),'#','\' || '#'))|| '#' ||  UPPER(REPLACE(COALESCE(TRIM( "stg_temp_src"."gender"),'~'),'#','\' || '#'))|| '#','#' || '~'),'~') ,'MD5'),'HEX')) AS "hash_diff"
			...
			, "stg_temp_src"."national_person_id" AS "national_person_id"
			, "stg_temp_src"."first_name" AS "first_name"
			, "stg_temp_src"."last_name" AS "last_name"
		...
		SELECT 
			...
			, "sat_src"."national_person_id" AS "national_person_id"
		...
		FROM "moto_scn01_fl"."sat_ms_customers_name" "sat_src"
	...
	SELECT 
		...
		, "temp_table_set"."national_person_id" AS "national_person_id"
		, "temp_table_set"."first_name" AS "first_name"
		, "temp_table_set"."last_name" AS "last_name"
...
BEGIN -- sat_inur_tgt
	...
	SELECT 
		...
		, "sat_temp_src_inur"."customer_number" AS "customer_number"
		, "sat_temp_src_inur"."national_person_id" AS "national_person_id"
		, "sat_temp_src_inur"."first_name" AS "first_name"
		, "sat_temp_src_inur"."last_name" AS "last_name"
		, "sat_temp_src_inur"."gender" AS "gender"
		, "sat_temp_src_inur"."update_timestamp" AS "update_timestamp"
	FROM "moto_sales_scn01_stg"."sat_ms_customers_name_tmp" "sat_temp_src_inur"
...
BEGIN -- sat_ed_tgt
	...
	( 
		SELECT 
			...
			, COALESCE(LEAD("sat_temp_src_us"."load_date",1)OVER(PARTITION BY "sat_temp_src_us"."customers_hkey" ORDER BY "sat_temp_src_us"."load_date")
				, TO_TIMESTAMP('31/12/2399 23:59:59.000000', 'DD/MM/YYYY HH24:MI:SS.US'::varchar)) AS "load_end_date"
		FROM "moto_sales_scn01_stg"."sat_ms_customers_name_tmp" "sat_temp_src_us"
	...
	UPDATE "moto_scn01_fl"."sat_ms_customers_name" "sat_ed_tgt"
	SET 
