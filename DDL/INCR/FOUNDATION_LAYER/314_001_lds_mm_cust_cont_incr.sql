CREATE OR REPLACE FUNCTION "moto_scn01_proc"."lds_mm_cust_cont_incr"() 
RETURNS void 
LANGUAGE 'plpgsql' 
...
BEGIN -- lds_inr_tgt
	...
	LEFT OUTER JOIN "moto_scn01_fl"."lds_mm_cust_cont" "lds_inr_src" ON  "stg_dl_inr_src"."lnd_cust_cont_hkey" = "lds_inr_src"."lnd_cust_cont_hkey" AND "stg_dl_inr_src"."load_date" = 
		"lds_inr_src"."load_date"
	WHERE  "stg_dl_inr_src"."operation" = 'I' AND "lds_inr_src"."lnd_cust_cont_hkey" IS NULL AND "stg_dl_inr_src"."error_code_cust_cont" = 0
	;
END;
...
BEGIN -- lds_temp_tgt
	...
	TRUNCATE TABLE "moto_mktg_scn01_stg"."lds_mm_cust_cont_tmp"  CASCADE;

	INSERT INTO "moto_mktg_scn01_stg"."lds_mm_cust_cont_tmp"(
		 "lnd_cust_cont_hkey"
		,"customers_hkey"
		,"contacts_hkey"
		,"party_number"
	...
	( 
		SELECT 
			  "stg_dis_src"."lnd_cust_cont_hkey" AS "lnd_cust_cont_hkey"
			, "stg_dis_src"."load_cycle_id" AS "load_cycle_id"
			, MIN("stg_dis_src"."load_date") AS "min_load_timestamp"
			, MIN("stg_dis_src"."error_code_cust_cont") AS "error_code_cust_cont"
		FROM "moto_mktg_scn01_stg"."party_contacts" "stg_dis_src"
		WHERE  "stg_dis_src"."error_code_cust_cont" = 0
		GROUP BY  "stg_dis_src"."lnd_cust_cont_hkey",  "stg_dis_src"."load_cycle_id"
	)
	, "last_lds" AS 
	( 
		SELECT 
			  "last_lds_src"."lnd_cust_cont_hkey" AS "lnd_cust_cont_hkey"
			, "dist_stg"."load_cycle_id" AS "load_cycle_id"
			, MAX("last_lds_src"."load_date") AS "max_load_timestamp"
			, MIN("dist_stg"."error_code_cust_cont") AS "error_code_cust_cont"
		FROM "moto_scn01_fl"."lds_mm_cust_cont" "last_lds_src"
		INNER JOIN "moto_scn01_fl"."lnd_cust_cont" "last_lnd_src" ON  "last_lds_src"."lnd_cust_cont_hkey" = "last_lnd_src"."lnd_cust_cont_hkey"
		INNER JOIN "dist_stg" "dist_stg" ON  "last_lnd_src"."lnd_cust_cont_hkey" = "dist_stg"."lnd_cust_cont_hkey"
		WHERE  "last_lds_src"."load_date" <= "dist_stg"."min_load_timestamp"
		GROUP BY  "last_lds_src"."lnd_cust_cont_hkey",  "dist_stg"."load_cycle_id"
	)
	, "temp_table_set" AS 
	( 
		SELECT 
			  "stg_temp_src"."lnd_cust_cont_hkey" AS "lnd_cust_cont_hkey"
		...
		FROM "moto_mktg_scn01_stg"."party_contacts" "stg_temp_src"
		WHERE  "stg_temp_src"."error_code_cust_cont" = 0
		UNION ALL 
		SELECT 
			  "lds_src"."lnd_cust_cont_hkey" AS "lnd_cust_cont_hkey"
		...
		FROM "moto_scn01_fl"."lds_mm_cust_cont" "lds_src"
		INNER JOIN "moto_scn01_fl"."lnd_cust_cont" "lnd_src" ON  "lds_src"."lnd_cust_cont_hkey" = "lnd_src"."lnd_cust_cont_hkey"
		INNER JOIN "last_lds" "last_lds" ON  "lds_src"."lnd_cust_cont_hkey" = "last_lds"."lnd_cust_cont_hkey"
		WHERE ( "lds_src"."load_end_date" = TO_TIMESTAMP('31/12/2399 23:59:59.000000' , 'DD/MM/YYYY HH24:MI:SS.US'::varchar) OR("last_lds"."error_code_cust_cont" = 0 AND "lds_src"."load_date" >= "last_lds"."max_load_timestamp"))AND "last_lds"."load_cycle_id" != "lds_src"."load_cycle_id"
	)
	SELECT 
		  "temp_table_set"."lnd_cust_cont_hkey" AS "lnd_cust_cont_hkey"
		...
		, CASE WHEN "temp_table_set"."source" = 'STG' AND "temp_table_set"."delete_flag"::text || "temp_table_set"."hash_diff"::text =
			LAG("temp_table_set"."delete_flag"::text || "temp_table_set"."hash_diff"::text,1)OVER(PARTITION BY "temp_table_set"."lnd_cust_cont_hkey" ORDER BY "temp_table_set"."load_date","temp_table_set"."origin_id")THEN 1 ELSE 0 END AS "equal"
...
BEGIN -- lds_inur_tgt
	...
	SELECT 
		  "lds_temp_src_inur"."lnd_cust_cont_hkey" AS "lnd_cust_cont_hkey"
	...
	FROM "moto_mktg_scn01_stg"."lds_mm_cust_cont_tmp" "lds_temp_src_inur"
...
BEGIN -- lds_ed_tgt
	...
	( 
		SELECT 
			  "lds_temp_src_us"."lnd_cust_cont_hkey" AS "lnd_cust_cont_hkey"
			...
			, COALESCE(LEAD("lds_temp_src_us"."load_date",1)OVER(PARTITION BY "lds_temp_src_us"."lnd_cust_cont_hkey" ORDER BY "lds_temp_src_us"."load_date",
				"lds_temp_src_us"."load_cycle_id"), TO_TIMESTAMP('31/12/2399 23:59:59.000000', 'DD/MM/YYYY HH24:MI:SS.US'::varchar)) AS "load_end_date"
		FROM "moto_mktg_scn01_stg"."lds_mm_cust_cont_tmp" "lds_temp_src_us"
	...
	( 
		SELECT 
			  "calc_load_end_date"."lnd_cust_cont_hkey" AS "lnd_cust_cont_hkey"
	...
	UPDATE "moto_scn01_fl"."lds_mm_cust_cont" "lds_ed_tgt"
	SET 
	...
	WHERE "lds_ed_tgt"."lnd_cust_cont_hkey" =  "filter_load_end_date"."lnd_cust_cont_hkey"
