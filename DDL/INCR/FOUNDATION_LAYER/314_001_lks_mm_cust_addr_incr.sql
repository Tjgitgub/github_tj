CREATE OR REPLACE FUNCTION "moto_scn01_proc"."lks_mm_cust_addr_incr"() 
RETURNS void 
LANGUAGE 'plpgsql' 
...
BEGIN -- lks_inr_tgt
	...
	INSERT INTO "moto_scn01_fl"."lks_mm_cust_addr"(
		 "lnk_cust_addr_hkey"
		,"addresses_hkey"
		,"customers_hkey"
		,"load_date"
	...
	SELECT 
		  "stg_inr_src"."lnk_cust_addr_hkey" AS "lnk_cust_addr_hkey"
	...
	LEFT OUTER JOIN "moto_scn01_fl"."lks_mm_cust_addr" "lks_inr_src" ON  "stg_inr_src"."lnk_cust_addr_hkey" = "lks_inr_src"."lnk_cust_addr_hkey" AND "stg_inr_src"."load_date" = 
		"lks_inr_src"."load_date"
	WHERE  "stg_inr_src"."operation" = 'I' AND "lks_inr_src"."lnk_cust_addr_hkey" IS NULL AND "stg_inr_src"."error_code_cust_addr" = 0
	;
END;
...
BEGIN -- lks_temp_tgt
	...
	TRUNCATE TABLE "moto_mktg_scn01_stg"."lks_mm_cust_addr_tmp"  CASCADE;

	INSERT INTO "moto_mktg_scn01_stg"."lks_mm_cust_addr_tmp"(
		 "lnk_cust_addr_hkey"
		,"addresses_hkey"
		,"customers_hkey"
		,"load_date"
	...
	( 
		SELECT 
			...
			, MIN("stg_dis_src"."error_code_cust_addr") AS "error_code_cust_addr"
		FROM "moto_mktg_scn01_stg"."party" "stg_dis_src"
		WHERE  "stg_dis_src"."error_code_cust_addr" = 0
	...
	( 
		SELECT 
			...
			, MIN("dist_stg"."error_code_cust_addr") AS "error_code_cust_addr"
		FROM "moto_scn01_fl"."lks_mm_cust_addr" "last_lks_src"
		INNER JOIN "moto_scn01_fl"."lnk_cust_addr" "last_lnk_src" ON  "last_lks_src"."lnk_cust_addr_hkey" = "last_lnk_src"."lnk_cust_addr_hkey"
	...
	( 
		SELECT 
			  "stg_temp_src"."lnk_cust_addr_hkey" AS "lnk_cust_addr_hkey"
		...
		WHERE  "stg_temp_src"."error_code_cust_addr" = 0
		UNION 
		SELECT 
			  "lks_src"."lnk_cust_addr_hkey" AS "lnk_cust_addr_hkey"
			, "lnk_src"."addresses_hkey" AS "addresses_hkey"
			...
			, "lks_src"."address_number" AS "address_number"
		FROM "moto_scn01_fl"."lks_mm_cust_addr" "lks_src"
		INNER JOIN "moto_scn01_fl"."lnk_cust_addr" "lnk_src" ON  "lks_src"."lnk_cust_addr_hkey" = "lnk_src"."lnk_cust_addr_hkey"
		INNER JOIN "last_lks" "last_lks" ON  "lnk_src"."customers_hkey" = "last_lks"."customers_hkey"
		WHERE ( "lks_src"."load_end_date" = TO_TIMESTAMP('31/12/2399 23:59:59.000000' , 'DD/MM/YYYY HH24:MI:SS.US'::varchar) OR("last_lks"."error_code_cust_addr" = 0 AND "lks_src"."load_date" >= "last_lks"."max_load_timestamp"))AND "last_lks"."load_cycle_id" != "lks_src"."load_cycle_id"
	)
	SELECT 
		  "temp_table_set"."lnk_cust_addr_hkey" AS "lnk_cust_addr_hkey"
		...
		, CASE WHEN "temp_table_set"."source" = 'STG' AND "temp_table_set"."delete_flag"::text || "temp_table_set"."addresses_hkey"::text =
			LAG("temp_table_set"."delete_flag"::text || "temp_table_set"."addresses_hkey"::text,1)OVER(PARTITION BY "temp_table_set"."customers_hkey" ORDER BY "temp_table_set"."load_date","temp_table_set"."origin_id")THEN 1 ELSE 0 END AS "equal"
...
BEGIN -- lks_inur_tgt
	...
	SELECT 
		  "lks_temp_src_inur"."lnk_cust_addr_hkey" AS "lnk_cust_addr_hkey"
		, "lks_temp_src_inur"."addresses_hkey" AS "addresses_hkey"
		...
		, "lks_temp_src_inur"."address_number" AS "address_number"
	FROM "moto_mktg_scn01_stg"."lks_mm_cust_addr_tmp" "lks_temp_src_inur"
...
BEGIN -- lks_ed_tgt
	...
	( 
		SELECT 
			  "lks_temp_src_us"."lnk_cust_addr_hkey" AS "lnk_cust_addr_hkey"
		...
		FROM "moto_mktg_scn01_stg"."lks_mm_cust_addr_tmp" "lks_temp_src_us"
	...
	( 
		SELECT 
			  "calc_load_end_date"."lnk_cust_addr_hkey" AS "lnk_cust_addr_hkey"
	...
	UPDATE "moto_scn01_fl"."lks_mm_cust_addr" "lks_ed_tgt"
	SET 
	...
	WHERE "lks_ed_tgt"."lnk_cust_addr_hkey" =  "filter_load_end_date"."lnk_cust_addr_hkey"
