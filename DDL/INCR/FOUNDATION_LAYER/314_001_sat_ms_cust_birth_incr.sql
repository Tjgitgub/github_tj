CREATE OR REPLACE FUNCTION "moto_scn01_proc"."sat_ms_cust_birth_incr"() 
RETURNS void 
LANGUAGE 'plpgsql' 
...
BEGIN -- sat_temp_tgt
	...
	TRUNCATE TABLE "moto_sales_scn01_stg"."sat_ms_customers_birth_tmp"  CASCADE;

	INSERT INTO "moto_sales_scn01_stg"."sat_ms_customers_birth_tmp"(
		 "customers_hkey"
		,"load_date"
		,"load_end_date"
		,"load_cycle_id"
	...
	( 
		SELECT 
			...
			, 'STG' AS "source"
			, 1 AS "origin_id"
			, UPPER(ENCODE(DIGEST(COALESCE(RTRIM( UPPER(REPLACE(COALESCE(TRIM( TO_CHAR("stg_temp_src"."birthdate", 
		...
		FROM "moto_scn01_fl"."sat_ms_customers_birth" "sat_src"
...
BEGIN -- sat_inur_tgt
	...
	FROM "moto_sales_scn01_stg"."sat_ms_customers_birth_tmp" "sat_temp_src_inur"
...
BEGIN -- sat_ed_tgt
	...
	( 
		SELECT 
			...
			, COALESCE(LEAD("sat_temp_src_us"."load_date",1)OVER(PARTITION BY "sat_temp_src_us"."customers_hkey" ORDER BY "sat_temp_src_us"."load_date")
				, TO_TIMESTAMP('31/12/2399 23:59:59.000000', 'DD/MM/YYYY HH24:MI:SS.US'::varchar)) AS "load_end_date"
		FROM "moto_sales_scn01_stg"."sat_ms_customers_birth_tmp" "sat_temp_src_us"
	...
	UPDATE "moto_scn01_fl"."sat_ms_customers_birth" "sat_ed_tgt"
	SET 
