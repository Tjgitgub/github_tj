CREATE OR REPLACE FUNCTION "moto_scn01_proc"."sat_ms_addr_incr"() 
RETURNS void 
LANGUAGE 'plpgsql' 
...
BEGIN -- sat_temp_tgt
	...
	TRUNCATE TABLE "moto_sales_scn01_stg"."sat_ms_addresses_tmp"  CASCADE;

	INSERT INTO "moto_sales_scn01_stg"."sat_ms_addresses_tmp"(
		 "addresses_hkey"
		,"load_date"
		,"load_end_date"
		,"load_cycle_id"
	...
	( 
		...
		FROM "moto_sales_scn01_stg"."addresses" "stg_dis_src"
	...
	( 
		SELECT 
			...
			, 'STG' AS "source"
			, 1 AS "origin_id"
			, UPPER(ENCODE(DIGEST(COALESCE(RTRIM( UPPER(REPLACE(COALESCE(TRIM( "stg_temp_src"."address_number"::text),'~')
				,'#','\' || '#'))|| '#' ||  UPPER(REPLACE(COALESCE(TRIM( "stg_temp_src"."coordinates"::text),'~'),'#','\' || '#'))|| '#','#' || '~'),'~') ,'MD5'),'HEX')) AS "hash_diff"
			...
			, "stg_temp_src"."coordinates" AS "coordinates"
			, "stg_temp_src"."update_timestamp" AS "update_timestamp"
		FROM "moto_sales_scn01_stg"."addresses" "stg_temp_src"
		...
		FROM "moto_scn01_fl"."sat_ms_addresses" "sat_src"
		INNER JOIN "dist_stg" "dist_stg" ON  "sat_src"."addresses_hkey" = "dist_stg"."addresses_hkey"
	...
	SELECT 
		...
		, "temp_table_set"."coordinates" AS "coordinates"
...
BEGIN -- sat_inur_tgt
	...
	SELECT 
		...
		, "sat_temp_src_inur"."coordinates" AS "coordinates"
		, "sat_temp_src_inur"."update_timestamp" AS "update_timestamp"
	FROM "moto_sales_scn01_stg"."sat_ms_addresses_tmp" "sat_temp_src_inur"
...
BEGIN -- sat_ed_tgt
	...
	( 
		SELECT 
			...
			, COALESCE(LEAD("sat_temp_src_us"."load_date",1)OVER(PARTITION BY "sat_temp_src_us"."addresses_hkey" ORDER BY "sat_temp_src_us"."load_date")
				, TO_TIMESTAMP('31/12/2399 23:59:59.000000', 'DD/MM/YYYY HH24:MI:SS.US'::varchar)) AS "load_end_date"
		FROM "moto_sales_scn01_stg"."sat_ms_addresses_tmp" "sat_temp_src_us"
	...
	UPDATE "moto_scn01_fl"."sat_ms_addresses" "sat_ed_tgt"
	SET 
