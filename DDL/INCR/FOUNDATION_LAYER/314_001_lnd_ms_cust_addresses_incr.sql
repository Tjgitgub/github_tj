CREATE OR REPLACE FUNCTION "moto_scn01_proc"."lnd_ms_cust_addresses_incr"() 
RETURNS void 
LANGUAGE 'plpgsql' 
...
BEGIN -- lnd_tgt
	...
	( 
		...
		WHERE  "stg_src1"."error_code_cust_addresses" in(0,1)
	)
	, "min_load_time" AS 
	( 
		SELECT 
			...
			, ROW_NUMBER()OVER(PARTITION BY "change_set"."lnd_cust_addresses_hkey" ORDER BY "change_set"."load_date",
	...
	LEFT OUTER JOIN "moto_scn01_fl"."lnd_cust_addresses" "lnd_src" ON  "min_load_time"."lnd_cust_addresses_hkey" = "lnd_src"."lnd_cust_addresses_hkey"
	WHERE  "lnd_src"."lnd_cust_addresses_hkey" IS NULL AND "min_load_time"."dummy" = 1
	;
END;
