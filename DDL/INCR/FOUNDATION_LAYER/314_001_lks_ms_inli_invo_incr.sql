CREATE OR REPLACE FUNCTION "moto_scn01_proc"."lks_ms_inli_invo_incr"() 
RETURNS void 
LANGUAGE 'plpgsql' 
...
BEGIN -- lks_temp_tgt
	...
	TRUNCATE TABLE "moto_sales_scn01_stg"."lks_ms_inli_invo_tmp"  CASCADE;

	INSERT INTO "moto_sales_scn01_stg"."lks_ms_inli_invo_tmp"(
		 "lnk_inli_invo_hkey"
		,"invoices_hkey"
		,"invoice_lines_hkey"
		,"load_date"
	...
	( 
		SELECT 
			  "stg_temp_src"."lnk_inli_invo_hkey" AS "lnk_inli_invo_hkey"
		...
		WHERE  "stg_temp_src"."record_type" != 'E'
		UNION 
		SELECT 
			  "lks_src"."lnk_inli_invo_hkey" AS "lnk_inli_invo_hkey"
		...
		FROM "moto_scn01_fl"."lks_ms_inli_invo" "lks_src"
		INNER JOIN "moto_scn01_fl"."lnk_inli_invo" "lnk_src" ON  "lks_src"."lnk_inli_invo_hkey" = "lnk_src"."lnk_inli_invo_hkey"
		INNER JOIN "dist_stg" "dist_stg" ON  "lnk_src"."invoice_lines_hkey" = "dist_stg"."invoice_lines_hkey"
		WHERE  "lks_src"."load_end_date" = TO_TIMESTAMP('31/12/2399 23:59:59.000000' , 'DD/MM/YYYY HH24:MI:SS.US'::varchar)
	)
	SELECT 
		  "temp_table_set"."lnk_inli_invo_hkey" AS "lnk_inli_invo_hkey"
		...
		, CASE WHEN "temp_table_set"."source" = 'STG' AND "temp_table_set"."delete_flag"::text || "temp_table_set"."invoices_hkey"::text =
			LAG( "temp_table_set"."delete_flag"::text || "temp_table_set"."invoices_hkey"::text,1)OVER(PARTITION BY "temp_table_set"."invoice_lines_hkey" ORDER BY "temp_table_set"."load_date","temp_table_set"."origin_id")THEN 1 ELSE 0 END AS "equal"
...
BEGIN -- lks_inur_tgt
	...
	INSERT INTO "moto_scn01_fl"."lks_ms_inli_invo"(
		 "lnk_inli_invo_hkey"
		,"invoices_hkey"
		,"invoice_lines_hkey"
		,"load_date"
	...
	SELECT 
		  "lks_temp_src_inur"."lnk_inli_invo_hkey" AS "lnk_inli_invo_hkey"
	...
	FROM "moto_sales_scn01_stg"."lks_ms_inli_invo_tmp" "lks_temp_src_inur"
...
BEGIN -- lks_ed_tgt
	...
	( 
		SELECT 
			  "lks_temp_src_us"."lnk_inli_invo_hkey" AS "lnk_inli_invo_hkey"
		...
		FROM "moto_sales_scn01_stg"."lks_ms_inli_invo_tmp" "lks_temp_src_us"
	...
	( 
		SELECT 
			  "calc_load_end_date"."lnk_inli_invo_hkey" AS "lnk_inli_invo_hkey"
	...
	UPDATE "moto_scn01_fl"."lks_ms_inli_invo" "lks_ed_tgt"
	SET 
	...
	WHERE "lks_ed_tgt"."lnk_inli_invo_hkey" =  "filter_load_end_date"."lnk_inli_invo_hkey"
