CREATE OR REPLACE FUNCTION "project1_proc"."sat_s1_table1_incr"() 
RETURNS void 
LANGUAGE 'plpgsql' 
...
BEGIN -- sat_temp_tgt
	...
	TRUNCATE TABLE "dv_0114_fl"."sat_s1_table1_tmp"  CASCADE;

	INSERT INTO "dv_0114_fl"."sat_s1_table1_tmp"(
		 "table1_hkey"
		,"load_date"
		,"load_end_date"
		,"load_cycle_id"
	...
	( 
		SELECT DISTINCT 
 			  "stg_dis_src"."table1_hkey" AS "table1_hkey"
			, "stg_dis_src"."load_cycle_id" AS "load_cycle_id"
		FROM "source1_stg"."table1" "stg_dis_src"
		WHERE  "stg_dis_src"."record_type" = 'S'
	)
	, "temp_table_set" AS 
	( 
		SELECT 
			  "stg_temp_src"."table1_hkey" AS "table1_hkey"
			...
			, 'STG' AS "source"
			, 1 AS "origin_id"
			, UPPER(ENCODE(DIGEST(COALESCE(RTRIM( REPLACE(COALESCE(TRIM( "stg_temp_src"."table1_id"::text),'~'),'#','\' || 
				'#')|| '#' ||  REPLACE(COALESCE(TRIM( "stg_temp_src"."some_attr"::text),'~'),'#','\' || '#')|| '#','#' || '~'),'~') ,'MD5'),'HEX')) AS "hash_diff"
			, CASE WHEN "stg_temp_src"."jrn_flag" = 'D' THEN 'Y'::text ELSE 'N'::text END AS "delete_flag"
			, "stg_temp_src"."other_attr" AS "other_attr"
			, "stg_temp_src"."table1_id" AS "table1_id"
			, "stg_temp_src"."some_attr" AS "some_attr"
		FROM "source1_stg"."table1" "stg_temp_src"
		WHERE  "stg_temp_src"."record_type" = 'S'
		UNION ALL 
		SELECT 
			  "sat_src"."table1_hkey" AS "table1_hkey"
			...
			, "sat_src"."table1_id" AS "table1_id"
			, "sat_src"."some_attr" AS "some_attr"
		FROM "dv_0114_fl"."sat_s1_table1" "sat_src"
		INNER JOIN "dist_stg" "dist_stg" ON  "sat_src"."table1_hkey" = "dist_stg"."table1_hkey"
		WHERE  "sat_src"."load_end_date" = TO_TIMESTAMP('31/12/2999 23:59:59' , 'DD/MM/YYYY HH24:MI:SS'::varchar)
	)
	SELECT 
		  "temp_table_set"."table1_hkey" AS "table1_hkey"
		...
		, CASE WHEN "temp_table_set"."source" = 'STG' AND "temp_table_set"."delete_flag"::text || "temp_table_set"."hash_diff"::text =
			LAG( "temp_table_set"."delete_flag"::text || "temp_table_set"."hash_diff"::text,1)OVER(PARTITION BY "temp_table_set"."table1_hkey" ORDER BY "temp_table_set"."load_date","temp_table_set"."origin_id")THEN 1 ELSE 0 END AS "equal"
		, "temp_table_set"."hash_diff" AS "hash_diff"
		, "temp_table_set"."delete_flag" AS "delete_flag"
		, "temp_table_set"."other_attr" AS "other_attr"
		, "temp_table_set"."table1_id" AS "table1_id"
...
BEGIN -- sat_inur_tgt
	...
	INSERT INTO "dv_0114_fl"."sat_s1_table1"(
		 "table1_hkey"
		,"load_date"
		,"load_end_date"
		,"load_cycle_id"
	...
	SELECT 
		  "sat_temp_src_inur"."table1_hkey" AS "table1_hkey"
		...
		, "sat_temp_src_inur"."table1_id" AS "table1_id"
		, "sat_temp_src_inur"."some_attr" AS "some_attr"
	FROM "dv_0114_fl"."sat_s1_table1_tmp" "sat_temp_src_inur"
...
BEGIN -- sat_ed_tgt
	...
	( 
		SELECT 
			  "sat_temp_src_us"."table1_hkey" AS "table1_hkey"
			, "sat_temp_src_us"."load_date" AS "load_date"
			, COALESCE(LEAD("sat_temp_src_us"."load_date",1)OVER(PARTITION BY "sat_temp_src_us"."table1_hkey" ORDER BY "sat_temp_src_us"."load_date")
				, TO_TIMESTAMP('31/12/2999 23:59:59', 'DD/MM/YYYY HH24:MI:SS'::varchar)) AS "load_end_date"
		FROM "dv_0114_fl"."sat_s1_table1_tmp" "sat_temp_src_us"
		WHERE  "sat_temp_src_us"."equal" = 0
	)
	, "filter_load_end_date" AS 
	( 
		SELECT 
			  "calc_load_end_date"."table1_hkey" AS "table1_hkey"
	...
	UPDATE "dv_0114_fl"."sat_s1_table1" "sat_ed_tgt"
	SET 
		 "load_end_date" =  "filter_load_end_date"."load_end_date"
	FROM  "filter_load_end_date"
	WHERE "sat_ed_tgt"."table1_hkey" =  "filter_load_end_date"."table1_hkey"
