CREATE OR REPLACE FUNCTION "moto_scn01_proc"."lnk_ms_inli_prod_incr"() 
RETURNS void 
LANGUAGE 'plpgsql' 
...
BEGIN -- lnk_tgt
	...
	( 
		...
		WHERE  "stg_src1"."error_code_inli_prod" in(0,1)
	)
	, "min_load_time" AS 
	( 
		SELECT 
			...
			, ROW_NUMBER()OVER(PARTITION BY "change_set"."lnk_inli_prod_hkey" ORDER BY "change_set"."load_date",
	...
	LEFT OUTER JOIN "moto_scn01_fl"."lnk_inli_prod" "lnk_src" ON  "min_load_time"."lnk_inli_prod_hkey" = "lnk_src"."lnk_inli_prod_hkey"
	WHERE  "lnk_src"."lnk_inli_prod_hkey" IS NULL AND "min_load_time"."dummy" = 1
	;
END;
