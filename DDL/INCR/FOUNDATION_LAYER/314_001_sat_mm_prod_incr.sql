CREATE OR REPLACE FUNCTION "moto_scn01_proc"."sat_mm_prod_incr"() 
RETURNS void 
LANGUAGE 'plpgsql' 
...
BEGIN -- sat_inr_tgt
	...
	LEFT OUTER JOIN "moto_scn01_fl"."sat_mm_products" "sat_inr_src" ON  "stg_inr_src"."products_hkey" = "sat_inr_src"."products_hkey" AND "stg_inr_src"."load_date" = "sat_inr_src"."load_date"
	WHERE  "stg_inr_src"."record_type" = 'S' AND "sat_inr_src"."products_hkey" IS NULL AND "stg_inr_src"."operation" = 'I'
	;
END;
...
BEGIN -- sat_temp_tgt
	...
	TRUNCATE TABLE "moto_mktg_scn01_stg"."sat_mm_products_tmp"  CASCADE;

	INSERT INTO "moto_mktg_scn01_stg"."sat_mm_products_tmp"(
		 "products_hkey"
		,"load_date"
		,"load_end_date"
		,"load_cycle_id"
	...
	( 
		...
		FROM "moto_mktg_scn01_stg"."motorcycles" "stg_dis_src"
	...
	( 
		...
		FROM "moto_mktg_scn01_stg"."motorcycles" "stg_temp_src"
		...
		SELECT 
			...
			, "sat_src"."motorcycle_id" AS "motorcycle_id"
			, "sat_src"."motorcycle_name" AS "motorcycle_name"
			, "sat_src"."update_timestamp" AS "update_timestamp"
		FROM "moto_scn01_fl"."sat_mm_products" "sat_src"
		INNER JOIN "dist_stg" "dist_stg" ON  "sat_src"."products_hkey" = "dist_stg"."products_hkey" AND "dist_stg"."load_cycle_id" != "sat_src"."load_cycle_id"
...
BEGIN -- sat_inur_tgt
	...
	SELECT 
		...
		, "sat_temp_src_inur"."motorcycle_id" AS "motorcycle_id"
		, "sat_temp_src_inur"."motorcycle_name" AS "motorcycle_name"
		, "sat_temp_src_inur"."update_timestamp" AS "update_timestamp"
	FROM "moto_mktg_scn01_stg"."sat_mm_products_tmp" "sat_temp_src_inur"
...
BEGIN -- sat_ed_tgt
	...
	( 
		SELECT 
			...
			, COALESCE(LEAD("sat_temp_src_us"."load_date",1)OVER(PARTITION BY "sat_temp_src_us"."products_hkey" ORDER BY "sat_temp_src_us"."load_date")
				, TO_TIMESTAMP('31/12/2399 23:59:59.000000', 'DD/MM/YYYY HH24:MI:SS.US'::varchar)) AS "load_end_date"
		FROM "moto_mktg_scn01_stg"."sat_mm_products_tmp" "sat_temp_src_us"
	...
	UPDATE "moto_scn01_fl"."sat_mm_products" "sat_ed_tgt"
	SET 
