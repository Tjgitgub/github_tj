CREATE OR REPLACE FUNCTION "moto_scn01_proc"."lds_mm_camp_prod_emo_incr"() 
RETURNS void 
LANGUAGE 'plpgsql' 
...
BEGIN -- lds_inr_tgt
	...
	INSERT INTO "moto_scn01_fl"."lds_mm_camp_prod_emo"(
		 "lnd_camp_prod_hkey"
		,"products_hkey"
		,"campaigns_hkey"
		,"campaign_code"
	...
	SELECT 
		...
		, UPPER(ENCODE(DIGEST(COALESCE(RTRIM( UPPER(REPLACE(COALESCE(TRIM( "stg_dl_inr_src"."motorcycle_emotion_desc"),
			'~'),'#','\' || '#'))|| '#' ||  UPPER(REPLACE(COALESCE(TRIM( "stg_dl_inr_src"."motorcycle_comment"),'~'),'#','\' || '#'))|| '#','#' || '~'),'~') ,'MD5'),'HEX')) AS "hash_diff"
		, 'N'::text AS "delete_flag"
		, "stg_dl_inr_src"."trans_timestamp" AS "trans_timestamp"
		, "stg_dl_inr_src"."motorcycle_emotion_desc" AS "motorcycle_emotion_desc"
		, "stg_dl_inr_src"."motorcycle_comment" AS "motorcycle_comment"
		, "stg_dl_inr_src"."update_timestamp" AS "update_timestamp"
	FROM "moto_mktg_scn01_stg"."campaign_motorcycles" "stg_dl_inr_src"
	LEFT OUTER JOIN "moto_scn01_fl"."lds_mm_camp_prod_emo" "lds_inr_src" ON  "stg_dl_inr_src"."lnd_camp_prod_hkey" = "lds_inr_src"."lnd_camp_prod_hkey" AND "stg_dl_inr_src"."load_date" = 
...
BEGIN -- lds_temp_tgt
	...
	TRUNCATE TABLE "moto_mktg_scn01_stg"."lds_mm_camp_prod_emo_tmp"  CASCADE;

	INSERT INTO "moto_mktg_scn01_stg"."lds_mm_camp_prod_emo_tmp"(
		 "lnd_camp_prod_hkey"
		,"products_hkey"
		,"campaigns_hkey"
		,"campaign_code"
	...
	( 
		...
		FROM "moto_scn01_fl"."lds_mm_camp_prod_emo" "last_lds_src"
	...
	( 
		SELECT 
			...
			, TO_TIMESTAMP('31/12/2399 23:59:59.000000', 'DD/MM/YYYY HH24:MI:SS.US'::varchar) AS "load_end_date"
			, UPPER(ENCODE(DIGEST(COALESCE(RTRIM( UPPER(REPLACE(COALESCE(TRIM( "stg_temp_src"."motorcycle_emotion_desc"),
				'~'),'#','\' || '#'))|| '#' ||  UPPER(REPLACE(COALESCE(TRIM( "stg_temp_src"."motorcycle_comment"),'~'),'#','\' || '#'))|| '#','#' || '~'),'~') ,'MD5'),'HEX')) AS "hash_diff"
		...
		FROM "moto_scn01_fl"."lds_mm_camp_prod_emo" "lds_src"
...
BEGIN -- lds_inur_tgt
	...
	FROM "moto_mktg_scn01_stg"."lds_mm_camp_prod_emo_tmp" "lds_temp_src_inur"
...
BEGIN -- lds_ed_tgt
	...
	( 
		...
		FROM "moto_mktg_scn01_stg"."lds_mm_camp_prod_emo_tmp" "lds_temp_src_us"
	...
	UPDATE "moto_scn01_fl"."lds_mm_camp_prod_emo" "lds_ed_tgt"
	SET 
