CREATE OR REPLACE FUNCTION "moto_scn01_proc"."sat_ms_prod_featu_class_incr"() 
RETURNS void 
LANGUAGE 'plpgsql' 
...
BEGIN -- sat_temp_tgt
	...
	TRUNCATE TABLE "moto_sales_scn01_stg"."sat_ms_prod_featu_class_tmp"  CASCADE;

	INSERT INTO "moto_sales_scn01_stg"."sat_ms_prod_featu_class_tmp"(
	...
	( 
		SELECT DISTINCT 
 			  "stg_dis_src"."prod_featu_class_hkey" AS "prod_featu_class_hkey"
			, "stg_dis_src"."load_cycle_id" AS "load_cycle_id"
		FROM "moto_sales_scn01_stg"."product_feature_class" "stg_dis_src"
	...
	( 
		SELECT 
			...
			, 'STG' AS "source"
			, 1 AS "origin_id"
			, UPPER(ENCODE(DIGEST(COALESCE(RTRIM( UPPER(REPLACE(COALESCE(TRIM( "stg_temp_src"."product_feature_class_desc")
		...
		FROM "moto_sales_scn01_stg"."product_feature_class" "stg_temp_src"
		WHERE  "stg_temp_src"."record_type" = 'S'
		UNION ALL 
		SELECT 
			  "sat_src"."prod_featu_class_hkey" AS "prod_featu_class_hkey"
		...
		FROM "moto_scn01_fl"."sat_ms_prod_featu_class" "sat_src"
		INNER JOIN "dist_stg" "dist_stg" ON  "sat_src"."prod_featu_class_hkey" = "dist_stg"."prod_featu_class_hkey"
	...
	SELECT 
		...
		, CASE WHEN "temp_table_set"."source" = 'STG' AND "temp_table_set"."delete_flag"::text || "temp_table_set"."hash_diff"::text =
			LAG( "temp_table_set"."delete_flag"::text || "temp_table_set"."hash_diff"::text,1)OVER(PARTITION BY "temp_table_set"."prod_featu_class_hkey" ORDER BY "temp_table_set"."load_date","temp_table_set"."origin_id")THEN 1 ELSE 0 END AS "equal"
...
BEGIN -- sat_inur_tgt
	...
	SELECT 
		  "sat_temp_src_inur"."prod_featu_class_hkey" AS "prod_featu_class_hkey"
	...
	FROM "moto_sales_scn01_stg"."sat_ms_prod_featu_class_tmp" "sat_temp_src_inur"
...
BEGIN -- sat_ed_tgt
	...
	( 
		SELECT 
			  "sat_temp_src_us"."prod_featu_class_hkey" AS "prod_featu_class_hkey"
			, "sat_temp_src_us"."load_date" AS "load_date"
			, "lci_src"."load_cycle_id" AS "load_cycle_id"
			, COALESCE(LEAD("sat_temp_src_us"."load_date",1)OVER(PARTITION BY "sat_temp_src_us"."prod_featu_class_hkey" ORDER BY "sat_temp_src_us"."load_date")
				, TO_TIMESTAMP('31/12/2399 23:59:59.000000', 'DD/MM/YYYY HH24:MI:SS.US'::varchar)) AS "load_end_date"
		FROM "moto_sales_scn01_stg"."sat_ms_prod_featu_class_tmp" "sat_temp_src_us"
	...
	( 
		SELECT 
			  "calc_load_end_date"."prod_featu_class_hkey" AS "prod_featu_class_hkey"
	...
	UPDATE "moto_scn01_fl"."sat_ms_prod_featu_class" "sat_ed_tgt"
	SET 
	...
	WHERE "sat_ed_tgt"."prod_featu_class_hkey" =  "filter_load_end_date"."prod_featu_class_hkey"
