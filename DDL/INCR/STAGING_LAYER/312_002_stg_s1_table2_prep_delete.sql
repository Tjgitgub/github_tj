CREATE OR REPLACE FUNCTION "project1_proc"."stg_s1_table2_prep_delete"() 
RETURNS void 
LANGUAGE 'plpgsql' 
...
BEGIN -- stg_prep_del_tgt
	...
	( 
		SELECT 
			  "stg_curr_ins"."table2_hkey" AS "table2_hkey"
			...
			, "stg_curr_ins"."table2_id" AS "table2_id"
			, "stg_curr_ins"."table2_id_bk" AS "table2_id_bk"
			, "stg_curr_ins"."some_attr" AS "some_attr"
			, "stg_curr_ins"."other_attr" AS "other_attr"
		FROM "source1_stg"."table2_curr" "stg_curr_ins"
		LEFT OUTER JOIN "source1_stg"."table2_prev" "stg_prev_ins" ON  "stg_prev_ins"."table2_hkey" = "stg_curr_ins"."table2_hkey"
		WHERE  "stg_prev_ins"."table2_hkey" IS NULL
		UNION ALL 
		SELECT 
			  "stg_curr_upd"."table2_hkey" AS "table2_hkey"
			...
			, "stg_curr_upd"."table2_id" AS "table2_id"
			, "stg_curr_upd"."table2_id_bk" AS "table2_id_bk"
			, "stg_curr_upd"."some_attr" AS "some_attr"
			, "stg_curr_upd"."other_attr" AS "other_attr"
		FROM "source1_stg"."table2_curr" "stg_curr_upd"
		INNER JOIN "source1_stg"."table2_prev" "stg_prev_upd" ON  "stg_prev_upd"."table2_hkey" = "stg_curr_upd"."table2_hkey"
		INNER JOIN "source1_mtd"."mtd_exception_records" "mex_null" ON  "mex_null"."record_type" = 'N'
		WHERE  COALESCE("stg_prev_upd"."table2_id", CAST("mex_null"."attribute_integer" AS INTEGER))!= COALESCE("stg_curr_upd"."table2_id", CAST("mex_null"."attribute_integer" AS INTEGER)) OR  COALESCE("stg_prev_upd"."some_attr", CAST("mex_null"."attribute_integer" AS INTEGER))!= COALESCE("stg_curr_upd"."some_attr", CAST("mex_null"."attribute_integer" AS INTEGER)) OR  COALESCE("stg_prev_upd"."other_attr", "mex_null"."attribute_varchar"::text)!= COALESCE("stg_curr_upd"."other_attr", "mex_null"."attribute_varchar"::text)
		UNION ALL 
		SELECT 
			  "stg_prev_del"."table2_hkey" AS "table2_hkey"
			, CURRENT_TIMESTAMP + row_number() over (PARTITION BY  "stg_prev_del"."table2_id_bk"  ORDER BY  "lci_src"."load_date")
				* interval'2 microsecond'   AS "load_date"
			, "lci_src"."load_cycle_id" AS "load_cycle_id"
			, "stg_prev_del"."src_bk" AS "src_bk"
			, 'D' ::text AS "jrn_flag"
			, "stg_prev_del"."record_type" AS "record_type"
			, "stg_prev_del"."table2_id" AS "table2_id"
			, "stg_prev_del"."table2_id_bk" AS "table2_id_bk"
			, "stg_prev_del"."some_attr" AS "some_attr"
			, "stg_prev_del"."other_attr" AS "other_attr"
		FROM "source1_stg"."table2_prev" "stg_prev_del"
		LEFT OUTER JOIN "source1_stg"."table2_curr" "stg_curr_del" ON  "stg_prev_del"."table2_hkey" = "stg_curr_del"."table2_hkey"
		INNER JOIN "source1_mtd"."load_cycle_info" "lci_src" ON  1 = 1
		WHERE  "stg_curr_del"."table2_hkey" IS NULL
	)
	SELECT 
		  "stg_curr_prev"."table2_hkey" AS "table2_hkey"
		...
		, "stg_curr_prev"."table2_id" AS "table2_id"
		, "stg_curr_prev"."table2_id_bk" AS "table2_id_bk"
