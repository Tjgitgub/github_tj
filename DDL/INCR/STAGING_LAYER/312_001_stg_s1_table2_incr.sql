CREATE OR REPLACE FUNCTION "project1_proc"."stg_s1_table2_incr"() 
RETURNS void 
LANGUAGE 'plpgsql' 
...
BEGIN -- stg_curr_to_prev_del
	...
	ALTER TABLE "source1_stg"."table2_prev" DISABLE TRIGGER ALL;

	DELETE  FROM "source1_stg"."table2_prev" "stg_del_tgt"
	WHERE  NOT EXISTS
	...
	(
		...
		INNER JOIN "source1_stg"."table2_curr" "stg_del_curr_src" ON  "lci_del_src"."load_cycle_id" = "stg_del_curr_src"."load_cycle_id"
	)
	;
END;
...
BEGIN -- stg_curr_to_prev
	...
	INSERT INTO "source1_stg"."table2_prev"(
		 "table2_hkey"
		,"load_date"
		,"load_cycle_id"
		,"src_bk"
...
BEGIN -- stg_tgt
	...
	SELECT 
		  "calc_hash_keys"."table2_hkey" AS "table2_hkey"
		...
		, "calc_hash_keys"."table2_id" AS "table2_id"
		, "calc_hash_keys"."table2_id_bk" AS "table2_id_bk"
