CREATE OR REPLACE FUNCTION "moto_scn01_proc"."stg_dl_ms_productfeatclassrel_incr"() 
RETURNS void 
LANGUAGE 'plpgsql' 
...
BEGIN -- stg_dl_tgt
	...
	( 
		SELECT 
			...
			, CASE WHEN "ext_src"."error_code_pro_fea_clas_rel" = 2 THEN 2 WHEN "ext_src"."error_code_pro_fea_clas_rel" =
				- 1 THEN 0 WHEN  "find_bk_fk1"."product_id" IS NULL OR "find_bk_fk2"."product_feature_class_id" IS NULL OR "find_bk_fk3"."product_feature_id" IS NULL  THEN "ext_src"."error_code_pro_fea_clas_rel" ELSE 0 END AS "error_code_pro_fea_clas_rel"
	...
	SELECT 
		  "calc_hash_keys"."lnd_pro_fea_clas_rel_hkey" AS "lnd_pro_fea_clas_rel_hkey"
		, "calc_hash_keys"."products_hkey" AS "products_hkey"
		, "calc_hash_keys"."prod_featu_class_hkey" AS "prod_featu_class_hkey"
		...
		, CASE WHEN "calc_hash_keys"."record_type" = 'E' THEN "calc_hash_keys"."load_date" ELSE CURRENT_TIMESTAMP +
			row_number() over (PARTITION BY  "calc_hash_keys"."products_hkey" ,  "calc_hash_keys"."product_features_hkey"  ORDER BY  "calc_hash_keys"."trans_timestamp") * interval'2 microsecond'   END AS "load_date"
		...
		, "calc_hash_keys"."product_feature_class_id" AS "product_feature_class_id"
		...
		, "calc_hash_keys"."pro_fea_cla_code_fk_productfeatureclassid_bk" AS "pro_fea_cla_code_fk_productfeatureclassid_bk"
		, "calc_hash_keys"."produ_featu_code_fk_productfeatureid_bk" AS "produ_featu_code_fk_productfeatureid_bk"
		, "calc_hash_keys"."update_timestamp" AS "update_timestamp"
		, "calc_hash_keys"."error_code_pro_fea_clas_rel" AS "error_code_pro_fea_clas_rel"
...
BEGIN -- ext_err_tgt
	...
	WHERE  "stg_err_src"."error_code_pro_fea_clas_rel" IN(1,3)AND "stg_err_src"."load_cycle_id" >= 0
	;
END;
