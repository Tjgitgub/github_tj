CREATE OR REPLACE FUNCTION "moto_scn01_proc"."ext_mm_phones_incr"() 
RETURNS void 
LANGUAGE 'plpgsql' 
...
BEGIN -- ext_tgt
	...
	TRUNCATE TABLE "moto_mktg_scn01_ext"."phones"  CASCADE;

	INSERT INTO "moto_mktg_scn01_ext"."phones"(
		 "load_cycle_id"
		,"load_date"
		,"trans_timestamp"
		,"operation"
	...
	( 
		SELECT 
			...
			, "tdfv_src"."phone_number" AS "phone_number"
			, "tdfv_src"."update_timestamp" AS "update_timestamp"
			, CASE WHEN "tdfv_src"."operation" = 'U' AND "tdfv_src"."phone_number" = 'unchanged'::text THEN 0 ELSE 1 END AS "ch_phone_number"
			...
			, CASE WHEN "tdfv_src"."operation" = 'U' AND "tdfv_src"."phone_number" = 'unchanged'::text THEN 1 ELSE 0 END AS "nc_phone_number"
			, CASE WHEN "tdfv_src"."operation" = 'U' AND "tdfv_src"."update_timestamp" = TO_TIMESTAMP('01/01/1970 00:00:00',
				'DD/MM/YYYY HH24:MI:SS.US'::varchar) THEN 1 ELSE 0 END AS "nc_update_timestamp"
		FROM "moto_mktg_scn01_dfv"."vw_phones" "tdfv_src"
	...
	( 
		SELECT 
			...
			, "calculate_variables"."phone_number" AS "phone_number"
			, "calculate_variables"."update_timestamp" AS "update_timestamp"
			, CASE WHEN "calculate_variables"."operation" != 'U' THEN 0 ELSE SUM("calculate_variables"."ch_phone_number")
				OVER(PARTITION BY"calculate_variables"."contact_id" ORDER BY "calculate_variables"."load_date")END AS "ch_index_phone_number"
			...
			, "calculate_variables"."nc_phone_number" AS "nc_phone_number"
	...
	( 
		...
		INNER JOIN "moto_scn01_fl"."sat_mm_phones" "sat_src1" ON  "change_index"."contact_id" = "sat_src1"."contact_id"
	)
	, "previous_version" AS 
	( 
		SELECT 
	...
	( 
		SELECT 
			...
			, "change_index"."phone_number" AS "phone_number"
			, "change_index"."update_timestamp" AS "update_timestamp"
			, CASE WHEN "change_index"."nc_phone_number" = 0 THEN 0 ELSE SUM("change_index"."nc_phone_number")
				OVER(PARTITION BY"change_index"."contact_id","change_index"."ch_index_phone_number" ORDER BY "change_index"."load_date")END AS "lag_index_phone_number"
		...
		SELECT 
			...
			, "result_sat_values"."phone_number" AS "phone_number"
			, "result_sat_values"."update_timestamp" AS "update_timestamp"
			, 0 AS "lag_index_phone_number"
			, 0 AS "lag_index_update_timestamp"
		FROM "previous_version" "previous_version"
		INNER JOIN "moto_scn01_fl"."sat_mm_phones" "result_sat_values" ON  "previous_version"."contact_id" = "result_sat_values"."contact_id" AND "previous_version"."load_date" = 
	...
	( 
		SELECT 
			...
			, LAG("create_set_nc_values"."phone_number", "create_set_nc_values"."lag_index_phone_number"::int)
				OVER(PARTITION BY"create_set_nc_values"."contact_id" ORDER BY "create_set_nc_values"."load_date","create_set_nc_values"."order_operation") AS "phone_number"
	...
	( 
		SELECT 
			...
			, "result_set_nc_values"."phone_number" AS "phone_number"
	...
	SELECT 
		...
		, "calculate_bk"."phone_number" AS "phone_number"
