CREATE OR REPLACE FUNCTION "project1_proc"."ext_s1_table2_incr"() 
RETURNS void 
LANGUAGE 'plpgsql' 
...
BEGIN -- ext_tgt
	...
	( 
		SELECT 
			...
			, "tdfv_src"."table2_id" AS "table2_id"
			, COALESCE(UPPER( "tdfv_src"."table2_id"::text),"mex_src"."key_attribute_integer") AS "table2_id_bk"
			, "tdfv_src"."some_attr" AS "some_attr"
			, "tdfv_src"."other_attr" AS "other_attr"
		FROM "source1_dfv"."vw_table2" "tdfv_src"
	...
	( 
		SELECT 
			...
			, CURRENT_TIMESTAMP + row_number() over (PARTITION BY  "calculate_bk"."table2_id_bk"  ORDER BY  "lci_src"."load_date")
	...
	SELECT 
		...
		, "ext_union"."table2_id" AS "table2_id"
		, "ext_union"."table2_id_bk" AS "table2_id_bk"
